...
namespace CloudStreamForms
{
    ...
    public class CloudStreamCore
    {
        ...
        public interface IAnimeProvider
        {
            ...
            //List<string> GetAllLinks(int currentSeason, bool isDub);
                ...
                while (d.Contains(lookfor))
                {
                    ...
                    for (int i = 0; i < activeMovie.title.MALData.seasonData.Count; i++)
                    {
                        for (int q = 0; q < activeMovie.title.MALData.seasonData[i].seasons.Count; q++)
                        {
                            ...
                            if (ToLowerAndReplace(compareName) == ToLowerAndReplace(animeTitle) || ToLowerAndReplace(ms.engName.Replace(" ", "")) == ToLowerAndReplace(animeTitle) || containsSyno)
                            { //|| (animeTitle.ToLower().Replace(compareName.ToLower(), "").Length / (float)animeTitle.Length) < 0.3f) { // OVER 70 MATCH
                                print("FINISHED:::::" + slug);
        ...
        public class KissFreeAnimeProvider : BaseAnimeProvider
        {
            ...
            public override void FishMainLink(string year, TempThread tempThred, MALData malData)
            {
                if (isApiRequred && !ajaxNonce.IsClean())
                { // FOR API REQUESTS, LIKE QUICKSEARCH
                    GetApi();
                }
                    ...
                    if (title.ToLower().Replace(" ", "").StartsWith(search.ToLower().Replace(" ", "")))
                    {
                        ...
                        for (int z = 0; z < activeMovie.title.MALData.seasonData.Count; z++)
                        {
                            for (int q = 0; q < activeMovie.title.MALData.seasonData[z].seasons.Count; q++)
                            {
                                ...
                                if (startDate != "" && date != "")
                                {
                                    if (DateTime.Parse(startDate) == DateTime.Parse(date))
                                    { // THE SAME
                ...
                try
                {
                    ...
                    data - rating = "4.75"
                        ...
                        if ((isMovie && canMovie) || (!isMovie && canShow))
                        {
                            ...
                            if ((i1 == i2 || i1 == i2 - 1 || i1 == i2 + 1) && activeMovie.title.movieType == movie123.type && movie123.name.ToLower().Contains(activeMovie.title.name.ToLower()))
                            { // --- THE SAME ---
                              // counter = 10000;
        ...
        class GomoStreamProvider : BaseMovieProvier
        {
            ...
            public override void LoadLinksTSync(int episode, int season, int normalEpisode, bool isMovie, TempThread tempThred)
            {
                ...
                try
                {
                    ...
                    if (activeMovie.title.movieType == MovieType.TVSeries)
                    { // ADD CORRECT FORMAT; https://gomostream.com/show/game-of-thrones/01-01
														...
														*/
                                                        ...
                                                        if (onlyURL != "" && onlyURL != "https://onlystream.tv")
                                                        {
                                                            try
                                                            {
                                                                ...
                                                                { // NEW USES JW PLAYER I THNIK, EASIER LINK EXTRACTION
                                                                    url = _url; valid = true;
                                                                }
                                                                else
                                                                { // OLD SYSTEM I THINK
                ...
                try
                {
                    ...
                    if (d != "" || !fetchData)
                    {
                        ...
                        try
                        {
                            ...
                            { // FISHING : THIS IS TO SPEED UP LINK FETHING
        ...
        public void GetEpisodeLink(int episode = -1, int season = 1, bool purgeCurrentLinkThread = true, bool onlyEpsCount = false, bool isDub = true)
        {
            ...
            StartThread("Get Links", () =>
            {
                try
                {
                    ...
                    if (animeSeach)
                    { // use https://www3.gogoanime.io/ or https://vidstreaming.io/
